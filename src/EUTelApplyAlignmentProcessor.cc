    ...
    _histogramSwitch(false)
{
    ...
    registerOptionalParameter("OutputReferenceCollection",
     ...
     registerOptionalParameter("ApplyToReferenceCollection",
             "Do you want the reference hit collection to be corrected by "\
             "the shifts and tilts from the alignment collection?", 
    ...
    registerProcessorParameter ("refhitCollectionNames",
            "List of refhit collections. First one is INPUT collection, "
            " every subsequent corresponds to applying alignment collection",
            _refhitCollectionSuffixes, _refhitCollectionSuffixExamples);

    registerOptionalParameter("DoAlignCollection",
            "Implement geometry shifts and rotations as described"\
            "in alignmentCollectionName ",
            _doAlignCollection, static_cast< bool > ( 0 ));

    registerOptionalParameter("DoGear",
    ...
    registerOptionalParameter("DoAlignmentInOneGo",
            "Apply alignment steps in one go. Is supposed to be "\
            "used for reversealignment in reverse order, like: "\
            "undoAlignment, undoPreAlignment, undoGear ",
    ...
    registerOptionalParameter("Alpha",
            "Rotation Angle around X axis",
            _alpha, static_cast< double > ( 0.00 ) );
    registerOptionalParameter("Beta",
            "Rotation Angle around Y axis",
            _beta, static_cast< double > ( 0.00 ) );
    registerOptionalParameter("Gamma",
            "Rotation Angle around Z axis",
            _gamma, static_cast< double > ( 0.00 ) );
    registerOptionalParameter("PrintEvents", 
...
void EUTelApplyAlignmentProcessor::CheckIOCollections(LCEvent* event)
{
    ...
    if(_applyToReferenceHitCollection) 
    {
        ...
        catch(...)
        {
            if(_iEvt%1000 == 0) 
            {
                streamlog_out (DEBUG5) 
                    ...
                    << " could not be retrieved, creating a dummy one " 
                    << "(all elements are null) but it will be updated " 
                    << "with the prea-align database probably  " << std::endl;
            }
        ...
        catch(...)
        {
            ...
            streamlog_out(DEBUG5) 
                << "NOT found _outputReferenceHitVec [" 
                << _outputReferenceHitCollectionName.c_str() 
                << "] at " << _outputReferenceHitVec << std::endl;  
    ...
    try 
    {
        ...
        streamlog_out(DEBUG2) <<  " input ["<< _inputHitCollectionName 
            <<"] collection found on event " << event->getEventNumber() << std::endl;
        streamlog_out(DEBUG2) <<  " input ["<< _inputHitCollectionName 
            << "] collection found at " << _inputCollectionVec << std::endl;  
    } 
    catch(DataNotAvailableException& e) 
    {
        streamlog_out(DEBUG3) <<  "No input ["
            << _inputHitCollectionName <<"] collection found on event " 
    ...
    if(_alignmentCollectionName != "gear" )
    {
        try 
        {
            ...
            streamlog_out(DEBUG2) <<  "found Alignment ["
                << _alignmentCollectionName <<"] collection found on event " 
                << event->getEventNumber() << " at " << _alignmentCollectionVec << std::endl;
        } 
        catch (DataNotAvailableException& e) 
        {
            streamlog_out(DEBUG3) <<  "No Alignment ["<< _alignmentCollectionName 
                <<"] collection found on event " << event->getEventNumber()
    ...
    try
    {
        ...
        if(_outputCollectionVec == 0)
        {
            ...
            streamlog_out(DEBUG2) <<  " created new "  <<  _outputHitCollectionName  << std::endl;
        } 
        else
        {
    ...
    catch(...)
    {
        ...
        streamlog_out  ( DEBUG2 ) <<  " try catch. nevertheless created new " 
            << _outputHitCollectionName  << std::endl;
    }
}
...
void EUTelApplyAlignmentProcessor::processRunHeader(LCRunHeader * runHeader) 
{
    ...
    streamlog_out(MESSAGE6) << "Processing run header " << _iRun
    ...
    streamlog_out(MESSAGE6) << detectorName << " : " << detectorDescription;
    ...
    {
        streamlog_out(ERROR5) << "You must define at least one Alignment "
            << "Collection to start ApplyAlignment Processor" << std::endl;
        throw StopProcessingException(this);
    }
    else if(_alignmentCollectionSuffixes.size() == 1)
    {
        streamlog_out(MESSAGE5) << "Only one Alignment Collection has been specified" << std::endl;
        streamlog_out(MESSAGE5) << "Ignore other Suffix collections, proceed with the " 
            << " defaults given by _referenceHitCollectionName and _inputHitCollectionName " << std::endl ;
        ...
        streamlog_out(MESSAGE5) << " _alignmentCollectionNames[0] = " << _alignmentCollectionNames[0]  << std::endl;
        streamlog_out(MESSAGE5) << " _hitCollectionNames[0] = " << _hitCollectionNames[0]  << std::endl;
        streamlog_out(MESSAGE5) << " _refhitCollectionNames[0] = " << _refhitCollectionNames[0]  << std::endl;
    }
    else if( ( _alignmentCollectionSuffixes.size() != _hitCollectionSuffixes.size() )
            ...
            || ( _hitCollectionSuffixes.size() != _refhitCollectionSuffixes.size() )   )
    {
      ...
      streamlog_out(ERROR)<< " ***************************************************************************"<<std::endl;    
      streamlog_out(ERROR)<< " ***          dont't mess with the steering cards                        ***"<<std::endl;    
      streamlog_out(ERROR)<< " ***                                                                     ***"<<std::endl;    
      streamlog_out(ERROR)<< std::endl;
 
      for (unsigned i = 0; i < _alignmentCollectionSuffixes.size(); i++) 
      {
          streamlog_out(ERROR) << " _alignmentCollectionSuffixes["<<i<<"] : " << _alignmentCollectionSuffixes[i];    
      }
      for (unsigned i = 0; i < _hitCollectionSuffixes.size(); i++) 
      {
        streamlog_out(ERROR) << " _hitCollectionSuffixes["<<i<<"] : " << _hitCollectionSuffixes[i];    
      }
      for (unsigned i = 0; i < _refhitCollectionSuffixes.size(); i++) 
      {
          streamlog_out(ERROR) << " _refhitCollectionSuffixes["<<i<<"] : " << _refhitCollectionSuffixes[i];    
      }
 
      streamlog_out(ERROR)<< std::endl;
      streamlog_out(ERROR)<< " Input collection names are inconsistent in number of elements:" << std::endl;
      streamlog_out(ERROR)<< "\t alignment ["<< _alignmentCollectionSuffixes.size()<<"]" << std::endl;
      streamlog_out(ERROR)<< "\t hit ["      << _hitCollectionSuffixes.size()      <<"]" << std::endl;
      streamlog_out(ERROR)<< "\t refhit ["   <<_refhitCollectionSuffixes.size()    <<"]" << std::endl;
      streamlog_out(ERROR)<< " ***                                                                     ***"<<std::endl;    
      streamlog_out(ERROR)<< " ***************************************************************************"<<std::endl;    
      streamlog_out(ERROR)<< std::endl;
      throw StopProcessingException(this);
...
void EUTelApplyAlignmentProcessor::processEvent (LCEvent * event) 
{
    ...
    {   
        ...
        for(int i = static_cast<int>(_alignmentCollectionNames.size()) -1; i >= 0; --i) 
        {
            ...
            // Reverse order to apply: is this the first alignement constants to be applied?
            // Just keep track of the proper collection names... (JDC kind of mess, actually]
            if(i ==  static_cast<int>(_alignmentCollectionNames.size()) -1) 
            {
                ...
                _outputHitCollectionName    = _hitCollectionNames.at(i);
                _outputReferenceHitCollectionName = _refhitCollectionNames.at(i); 
            } 
            else
            {
            ...
            else if( _doAlignmentInOneGo == true )  // logic from 01.09.2012 . hopefully to declare PRO soon
            {                   
                if( _alignmentCollectionName == "gear" )
                {
            ...
            {
                streamlog_out ( ERROR5 ) << "You MUST specifiy whether you " 
                    << "want the alignment to be done in one go or not" << std::endl;
                streamlog_out ( ERROR5 ) << "Check your steering cards!!" << std::endl;
            }
        }
...
//   go-go
        ...
        {
          if(_isFirstEvent) streamlog_out( MESSAGE5 ) << "REVERSE: reference Hit collection name : " << _referenceHitCollectionName << " at " << _referenceHitVec  << endl;
...
void EUTelApplyAlignmentProcessor::TransformToLocalFrame(TrackerHitImpl* outputHit) 
{
          ...
          ClusterType cl_type = static_cast<ClusterType>(static_cast<int>(outputHit->getType()));
          if ( _iEvt < _printEvents )
          {
            streamlog_out ( DEBUG2 ) << "RevertGear: hit/cluster type:" << static_cast<int>(cl_type) << endl;
          }
          ...
          {      
            ...
            if ( cl_type == kEUTelSparseClusterImpl ) 
            {
            ...
            double cl_sign = 0;
            if      ( xPointing[0] < -0.7 )       cl_sign = -1 ;
            else if ( xPointing[0] > 0.7 )       cl_sign =  1 ;
            else 
            {
             if       ( xPointing[1] < -0.7 )    cl_sign = -1 ;
             else if  ( xPointing[1] > 0.7 )    cl_sign =  1 ;
            }
            telPos[0] +=  (-1)* cl_sign * xSize/2;   //apply shifts few lines later

            if      ( yPointing[0] < -0.7 )       cl_sign = -1 ;
            else if ( yPointing[0] > 0.7 )       cl_sign =  1 ;
            else 
            {
             if       ( yPointing[1] < -0.7 )    cl_sign = -1 ;
             else if  ( yPointing[1] > 0.7 )    cl_sign =  1 ;
            }
            telPos[1] += (-1)* cl_sign * ySize/2;   // apply shifts few lines later
