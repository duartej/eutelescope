    ...
    _missingCorrdinateValue(0)
{
    ...
    registerProcessorParameter("SensorIDStartsFrom",
            "The sensor ID for the data. The actual sensorID (alibava) will be stored"\
            " as SensorIDStartsFrom + ChipNumber ",
            _sensorIDStartsFrom, int(6) );
        
    // now the optional parameters
    registerOptionalParameter ("MissingCoordinateValue",
            "The value that should be stored in missing coordinate. This number has to be"\
            " integer since it will be used as channel number of the missing coordinate",
...
void AlibavaClusterConverter::processEvent (LCEvent * anEvent) 
{
    ...
    LCCollectionVec * alibavaCluColVec = nullptr; // alibava cluster collection vector
    try
    {
    ...
    const unsigned int noOfClusters = alibavaCluColVec->getNumberOfElements();
	...
	TrackerDataImpl * alibavaClu = dynamic_cast<TrackerDataImpl*>(alibavaCluColVec->getElementAt( i )) ;
        ...
        const int chipnum = anAlibavaCluster.getChipNum();
	...
	const int clusterSize = anAlibavaCluster.getClusterSize();
	for (int imember=0; imember < clusterSize; imember++) 
        {
            EUTelGenericSparsePixel eutelPixel;
				
	    const int memberChanNum = anAlibavaCluster.getChanNum(imember);
	    const float memberSignal = anAlibavaCluster.getSignal(imember) * anAlibavaCluster.getSignalPolarity();
