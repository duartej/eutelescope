...
{
    streamlog_out ( MESSAGE1 )  << "Reading HistoXMLFile : "<< _histoXMLFileName << std::endl;
    ...
    const EVENT::IntVec chipSelection = getChipSelection();

    std::string signalMultipliedby;
    if (_multiplySignalby != 1.0)
    {		
        signalMultipliedby = "( "+std::to_string(_multiplySignalby)+") ";
    }
    ...
    if(histoXMLDoc->LoadFile( _histoXMLFileName ) ) 
    {
        ...
        TiXmlElement * topTagElement = nullptr;
        ...
        {
            delete histoXMLDoc;
            throw ParseException(std::string("AlibavaBaseHistogramMaker::bookHistos no <") 
                    + _topTag + string("> ... </")+ _topTag +string("> block found in ")  + _histoXMLFileName);
        } 
	...
	TiXmlNode * histosBlockNode = handleTagToProcess.ToNode();
        if( !histosBlockNode ) 
        {
            delete histoXMLDoc;
            throw ParseException( std::string( "AlibavaBaseHistogramMaker::bookHistos no <") 
                    + _tagToProcess +string("> ... </")+_tagToProcess+string("> block found in ")  + _histoXMLFileName);
        }
        int xBin, yBin;
	...
	while (histoNode) {
		...
		if (perEachChip == string("true") || perEachChip == string("True")) {
			for (unsigned int i=0; i<chipSelection.size(); i++) {
				...
				if (histoType == string("TH1F")){
					histo1D = new TH1F (newHistoName.c_str(),"",xBin, xMin, xMax);
					histo1D->SetTitle(titleString.c_str());
					_rootObjectMap.insert(make_pair(newHistoName, histo1D));
				}
				else if (histoType == string("TH2F")){
					histo2D = new TH2F (newHistoName.c_str(),"",xBin, xMin, xMax, yBin, yMin, yMax);
			...
			if (histoType == string("TH1F")){
				histo1D = new TH1F (histoName.c_str(),"",xBin, xMin, xMax);
				histo1D->SetTitle(titleString.c_str());
				_rootObjectMap.insert(make_pair(histoName, histo1D));
			}
			else if (histoType == string("TH2F")){
				histo2D = new TH2F (histoName.c_str(),"",xBin, xMin, xMax, yBin, yMin, yMax);
				histo2D->SetTitle(titleString.c_str());
    ...
    {
        streamlog_out (MESSAGE1) << "All histograms needed are defined in "<<_histoXMLFileName<<std::endl;
    }
}
...
{
    const IntVec chipSelection = getChipSelection();
    for(unsigned int i=0; i<chipSelection.size(); ++i)
    {
        const int ichip=chipSelection[i];
...
bool AlibavaBaseHistogramMaker::checkListOfHistosCreatedByXMLFile()
{
    ...
    {
        std::string histoName = _histoList[ihisto];
        if(!doesRootObjectExists(histoName)) 
        {
            streamlog_out (ERROR5) << "Histogram "<< histoName <<" doesn't exists"<<std::endl;
...
void AlibavaBaseHistogramMaker::plotPedestalAndNoise()
{
    ...
    for (unsigned int i=0; i<chipSelection.size(); i++) {
    	...
    	if (_pedestalCollectionName != string(ALIBAVA::NOTSET) ) {
    		TH1F * pedestalHisto = new TH1F (getPedestalHistoName(ichip).c_str(),"",ALIBAVA::NOOFCHANNELS, -0.5, ALIBAVA::NOOFCHANNELS-0.5);
    	...
    	if (_noiseCollectionName != string(ALIBAVA::NOTSET) ) {
    		TH1F * noiseHisto = new TH1F (getNoiseHistoName(ichip).c_str(),"",ALIBAVA::NOOFCHANNELS, -0.5, ALIBAVA::NOOFCHANNELS-0.5);
