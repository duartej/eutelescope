    ...
    _temperatureVsEventNumHistoName("hEventTemperature_vs_EventNum")
{
    ...
    _description ="AlibavaDataHistogramMaker takes TrackerData of Alibava data and produces histograms ";
    ...
    registerProcessorParameter ("TagToProcess","The tag in TopTagInXMLFile. This processor "\
            "will only consider the histogram definitions inside this tag. This tag should "\
            "be inside <TopTagInXMLFile> ... <TopTagInXMLFile/>",
            _tagToProcess , string("myAlibavaDataHistogramMaker"));

    // optional parameters
    registerOptionalParameter ("PedestalInputFile",
            "The filename where the pedestal and noise values stored",_pedestalFile , string(ALIBAVA::NOTSET));
    
    registerOptionalParameter ("CalibrationInputFile", "The filename where the calibration values stored",
    ...
    registerOptionalParameter ("PlotPedestalAndNoise",
            "Choose if pedestal and noise should be plotted. If you want to "\
            "plot only noise or pedestal set this variable true and only set "\
            "the noise or pedestal collection name you want to be plotted.",_plotPedestalAndNoise, bool(false));
    registerOptionalParameter ("EventsToPlot","Choose if pedestal and noise "\
            "should be plotted. If you want to plot only noise or pedestal, set"\
            " this variable to true and only set the noise or pedestal collection "\
            " name you want to be plotted.",
            _plotEvents, IntVec());
    ...
    registerOptionalParameter ("PlotSomePercentOfEvents",
            "In case this variable is set (say x), x percent of total events will "\
            "be plotted randomly. The number should be between 0-100",
...
void AlibavaDataHistogramMaker::processEvent(LCEvent * anEvent) 
{
    ...
    const float temperature = alibavaEvent->getEventTemp();
    
    //////////////////////////////////////////////
    // fill the histograms common for the event //

    // TDC time
    TH1F* histo = dynamic_cast<TH1F*>(_rootObjectMap[_timeHistoName]);
    histo->Fill(tdctime);

    // TDC time vs EventNum
    TProfile *profile = dynamic_cast<TProfile*>(_rootObjectMap[_timeVsEventNumHistoName]);
    profile->Fill(eventnum, tdctime);
    
    // Temperature
    histo = dynamic_cast<TH1F*>(_rootObjectMap[_temperatureHistoName]);
    ...
    histo = dynamic_cast<TH1F*>(_rootObjectMap[_calChargeHistoName]);
    histo->Fill(alibavaEvent->getCalCharge());
    
    // Calibration delay values
    histo = dynamic_cast<TH1F*>(_rootObjectMap[_delayHistoName]);
    ...
    // loop over beetles
    for(int i=0; i<collectionVec->getNumberOfElements(); ++i)
    {
    	...
    	if(plotThisEvent)
        {
            this->fillEventHisto(eventnum,trkdata);
        }
        // for everything else
    	this->fillOtherHistos(trkdata, tdctime, temperature);
    }
}
...
void AlibavaDataHistogramMaker::fillOtherHistos(TrackerDataImpl * trkdata, float tdctime, float temperature)
{
    ...
    const int ichip = getChipNum(trkdata);
	
    FloatVec noiseVec;
    if(this->isNoiseValid())
    {
    ...
    for(int ichan=0; ichan<ALIBAVA::NOOFCHANNELS; ++ichan) 
    {
    	...
    	if(isNoiseValid() && noiseVec[ichan] != 0)
        {
...
void AlibavaDataHistogramMaker::bookEventHisto(int eventnum)
{
    ...
    {
        TH1F * eventHisto = new TH1F(this->getEventHistoName(eventnum,ichip).c_str(),
                "",ALIBAVA::NOOFCHANNELS, -0.5, ALIBAVA::NOOFCHANNELS-0.5);
        const std::string sp("Event "+std::to_string(eventnum)+" (chip "+std::to_string(ichip)+");"\
                "Channel Number;Signal (ADCs)");
        eventHisto->SetTitle(sp.c_str());
        _rootObjectMap.insert(std::make_pair(this->getEventHistoName(eventnum,ichip), eventHisto));
...
void AlibavaDataHistogramMaker::fillEventHisto(int eventnum, TrackerDataImpl * trkdata)
{
    ...
    TH1F * histo = dynamic_cast<TH1F*> (_rootObjectMap[getEventHistoName(eventnum,ichip)]);
    for(int ichan=0; ichan<ALIBAVA::NOOFCHANNELS; ichan++) 
    {
        // if channel is masked, do not fill histo
	if(isMasked(ichip,ichan))
...
void AlibavaDataHistogramMaker::createRulesToChangeXMLValues()
{
    ...
    this->changeXMLVariable(_timeVsEventNumHistoName, "xMax", float(_totalNumberOfEvents));
    this->changeXMLVariable(_temperatureVsEventNumHistoName, "xMax", float(_totalNumberOfEvents));
