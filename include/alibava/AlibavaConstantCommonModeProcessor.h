...
namespace alibava 
{
    ...
    class AlibavaConstantCommonModeProcessor:public alibava::AlibavaBaseProcessor   
    {
	     ...
	     */
	    void bookHistos(const int & TotalNevents);
	    
	    //! Fill histograms
	    /*! This method is used to fill in histograms for each channel. 
	     *
	     */
	    void fillHistos(const std::pair<EVENT::FloatVec,EVENT::FloatVec> & commonNoise,const int &  event,const int & chipnum);
	    ...
	    std::string getCommonModeErrorCollectionName();
            ...
            // get the name of the histograms depending the chip number
            std::string getHistoNameCMS(const int & ichip);
            std::string getHistoNameCMSError(const int & ichip);

            // Name of the histograms
            // common mode shift and error over events
            std::string _commonModeShiftHistoName;
            // Name of the histograms: common mode noise over events
            std::string _commonModeShiftErrorHistoName;

            //! Range for the ADCs  in the histograms
            float _adcmax;
            
            //! Range for the ADCs for the common mode error in the histograms 
            float _adcmax_err;
            //! Range for the ADCs  in the histograms
            float _adcmin_err;
