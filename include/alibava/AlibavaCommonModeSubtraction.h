...
namespace alibava 
{
    class AlibavaCommonModeSubtraction:public alibava::AlibavaBaseProcessor   
    {
	     ...
	     */
	    void bookHistos(const int & NtotalEvts);
	    
	    //! Fill histograms
	    /*! This method is used to fill in histograms for each channel. 
	     *
	     */
	    void fillHistos(const EVENT::FloatVec & datavec, const int & nevts, const int & chipnum);
	    ...
	    std::string _commonmodeerrorCollectionName;
            ...
            //! Corrected signal histograma name
            std::string _correctedSignalHistoName;

            //! Get the name of the histogram depending of the chip
            std::string getHistoNameCS(const int & ichip);
